...
線上互動驗收亦已完成。首頁可進入閱讀器；目錄顯示 305 篇；搜尋「蒹葭」會縮減為單一結果；選取後網址更新為 `/poems/129/`，並正確顯示〈蒹葭〉三章全文。

### 2026-09-11 功能擴充

本次新增首頁與目錄關鍵字搜尋，範圍涵蓋篇名、原文、白話譯文及註詞，並在結果中顯示命中節錄；每篇新增明確的「加入收藏」按鈕，收藏清單仍保存於瀏覽器本機。全部 305 篇新增逐章白話翻譯、3–6 項詞語註釋與閱讀提示，經 `gpt-5-mini` 初稿、`gpt-5` 編輯覆核及現代白話改寫，並保留「尚待人類學者逐篇核定」聲明。

社群圖依愛情、農事、征役、祭祀、宴飲、思歸使用六套水墨配色及背景題字；網站閱讀器同步採用低飽和主題背景。`dateModified` 與短版 SHA 改由 GitHub Actions 依部署提交自動注入，社群圖片網址亦帶 SHA 快取鍵。完整功能驗收紀錄見 `docs/FEATURE_QA.md`。
